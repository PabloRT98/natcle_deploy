...
function pdf24Plugin_queryBLinks() {
	global $pdf24Plugin;
	if(empty($pdf24Plugin['linkQueryUrl'])) {
		return array();
	}
...
function pdf24Plugin_isPrivateIp($ip) {
	...
	return $ip == 'localhost' || strpos($ip, '127.') === 0 || strpos($ip, '10.') === 0
		|| strpos($ip, '172.16.') === 0 || strpos($ip, '192.168.') === 0 || strpos($ip, '::') === 0;
}
...
function pdf24Plugin_getBLinks() {
	...
	$lastBlQuery = get_option('pdf24Plugin_lastBLinksQueryTime');
	if(!pdf24Plugin_isPrivateIp($_SERVER['SERVER_ADDR'])) {
		if(!$lastBlQuery || time() - intval($lastBlQuery) > 7 * 24 * 3600) {
	...
	if(!is_array($bLinks) || !$lastBlQuery || time() - intval($lastBlQuery) > 15 * 24 * 3600) {
		$bLinks = array();
	}
...
function pdf24Plugin_parseDocFilename($name) {	
	...
	$name = preg_replace_callback('/\{date(:(.*?))?}/', function($m) {
		$dp = 'Y-m-d';
		if(isset($m[2]) && $m[2]) {
			$dp = $m[2];
		}
